...
- [x] T001 `src/observer.rs`: extract a `pub(crate)` moving-body crossing helper —
...
- [x] T002 `src/lib.rs`: declare `sun` and `moon` modules, root re-exports per plan

## Phase 2: User Story 1 — Twilight (P1)

- [x] T003 [US1] (fresh, Meeus 25) `sun_position` in `src/sun.rs` + inline Meeus 25.a test
- [x] T004 [US1] (fresh) `Twilight`, `TwilightOutcome`, `twilight()` on the moving-body
  helper anchored at solar anti-transit; serde derives behind the feature
- [x] T005 [US1] Tests: Leiden 2026-10-15 astronomical dusk/dawn sanity (bracket midnight,
...
- [x] T006 [US2] (fresh, Meeus 47 tables) geocentric `moon_position` + `moon_distance_km`
  in `src/moon.rs` + inline Meeus 47.a pinned test (λ, β, Δ, π)
- [x] T007 [US2] (fresh, Meeus 40) `moon_position_topocentric` (WGS-84 site coords,
  elevation term) + parallax-magnitude inline test
- [x] T008 [US2] (fresh) `lunar_separation` (topocentric, target precessed of-date) and
  `moon_crossings` on the moving-body helper
- [x] T009 [US2] Integration tests in `tests/known_values.rs`: lunar separation vs a
...
- [x] T010 [US3] (fresh, Meeus 48) `moon_phase_angle` + `moon_illumination` + inline 48.a
  pinned test
- [x] T011 [US3] (fresh) `moon_avoidance_lorentzian` + exact S(0)=S, S(H)=S/2 tests;
  illumination-bounds and avoidance-decay properties in `tests/properties.rs`

## Phase 5: Polish & cross-cutting

- [x] T012 Extend `scripts/gen_astropy_vectors.py`: `get_sun`, `get_body("moon")`
...
- [x] T013 Extend `tests/astropy_vectors.rs` with the sun/moon/twilight/illumination
  sections at FR tolerances
- [x] T014 Serde round-trips for `Twilight`/`TwilightOutcome` in `tests/serde_feature.rs`;
  example `plan_night.rs` gains darkness window + lunar separation + illumination lines
- [x] T015 Docs: module provenance headers, README bullets, DECISIONS.md build log;
